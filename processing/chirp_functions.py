...
def fourier_transform(real_data, kys, kx = 0): #Just for y, so far
    ...
    for k in kys:
        amplitude = np.abs(np.array(
                [point[2] * np.exp(1j * (kx * (point[0] / 1000) + k * (point[1] / 1000) + (point[3] / 57.2957795131))) 
...
def display_chirp_scan(filename, calfilename, size, figsize, kxs, kys, reduce = 1.0, lfbeg = 0, lfend = 500):
    ...
    plt.clf()
    """
def fourier_chirps(size, freqs, kmin, kmax, ksamples, filename, sfilename,
    ...
    for setno in range(len(freqs)):
        for index, freq in tqdm.tqdm(list(enumerate(freqs[setno]))):
            ...
            result = fourier_transform(real_data = scan_data, kys = kvecs, kx = 0.0)
