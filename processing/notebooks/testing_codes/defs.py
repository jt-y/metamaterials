...
def organize_band_frequencies(
    dataset, max_bands=None, band_selection='bottom'
):
    """Sort each sweep's modes and retain the requested spectral end."""
    if band_selection not in ('top', 'bottom'):
        raise ValueError("band_selection must be either 'top' or 'bottom'")
    ...
    for b_value, frequencies in sorted(frequencies_by_b.items()):
        ...
        if max_bands is not None:
            if band_selection == 'top':
                sorted_frequencies = sorted_frequencies[-max_bands:]
            else:
                sorted_frequencies = sorted_frequencies[:max_bands]
        organized['frequencies_by_b_hz'][b_value] = sorted_frequencies
...
def extract_band_structure(
    csv_path, max_bands=None, band_selection='bottom'
):
    """Extract the top or bottom modes of each COMSOL sweep in frequency order."""
    return organize_band_frequencies(
        extract_eigenfrequencies(csv_path),
        max_bands=max_bands,
