...
class OscilloscopeMicrophone(object):
    ...
    def _record(self, n, sample_start=0, sample_end=10000, delay=RECORD_DELAY_TIME):
        ...
        while time.time() < end_time:
            try:
                ...
                # placing -1 values
                array = np.zeros(sample_end - sample_start) - 1
                lst.append(array.tolist())
                time.sleep(delay)
        return np.array(lst)
    ...
    def _fetch_fft_sample(self, sample_start, sample_end):
        ...
        data = np.array(self.device.query_ascii_values('CURVE?'))
        
        # CORRECTION DUE TO OSCILLOSCOPE ERROR
        
        """
        crtm = 0.0       
        if self.PREVIOUS_MAX > -4500.0:
            crtm = data.max()
            k = round((self.PREVIOUS_MAX - crtm) / 100.0)
            crtm+=(100.0*k)
            data = data + (100.0*k)
        
        # Add explanation for code above

        self.PREVIOUS_MAX = data.max()
        """
        #print(data.max())
        return data
