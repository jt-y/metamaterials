...
class Printer(object):
    ...
    def move_coord(self, x=None, y=None, z=None, speed=None):
        ...
        self.x_pos += x if x else 0.0
        self.y_pos += y if y else 0.0
        self.z_pos += z if z else 0.0
    ...
    def load_position_from_file(self):
        with open(os.path.join(configfolder, "position.pkl"), 'rb') as f:
            pos_array = pickle.load(f)

        self.x_pos = pos_array[0]
        self.y_pos = pos_array[1]
        self.z_pos = pos_array[2]

        print("Loaded position x = {}, y = {}, z = {} from config file".format(
            self.x_pos, self.y_pos, self.z_pos))
    
    def save_position_to_file(self):
        if not os.path.exists(configfolder):
            os.makedirs(configfolder)
        fname = os.path.join(configfolder,"position.pkl")
        pos = np.array([self.x_pos, self.y_pos, self.z_pos])
        pos.dump(fname)
        print("Saved current coordinates to file")

    def print_crt_position(self):
        print("Printer is at x = {}, y = {}, z = {}".format(self.x_pos, self.y_pos, self.z_pos))

    def move_to_abs(self, x = None, y = None, z = None, speed = 1000):
        
        if not x: x = self.x_pos 
        if not y: y = self.y_pos 
        if not z: z = self.z_pos 
        ...
        if z > 0:
            self.move_coord(z = z , speed = speed)
            time.sleep(z * 60.0 / speed)
            print("Moved to correct z") 
        
        self.move_coord(x = x, y = y, speed = speed)
        time.sleep(np.sqrt(x * x + y * y) * 60.0 / speed)

        if z < 0:
            self.move_coord(z = z , speed = speed)
            time.sleep(abs(z) * 60.0 / speed)
            print("Moved to correct z")
